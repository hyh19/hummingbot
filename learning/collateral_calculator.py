...
该命令行工具用于根据借出报价资产数量、初始质押率与质押资产的报价计算所需质押的基础资产数量。适用于杠杆借贷、场外撮合等需要快速评估抵押资金占用的场景。
...
程序会打印借出数量、初始质押率、质押资产价格以及所需的质押数量，便于复制到表格或报告中。所有金额与数量均采用用户指定的精度，确保与上游业务流程保持一致。
...
示例 1：默认精度
  python3 collateral_calculator.py --borrow-amount 20000 --borrow-asset usdt --initial-margin 0.6 --collateral-asset eth --collateral-price 3500

示例 2：指定输出位数
  python3 collateral_calculator.py --borrow-amount 50000 --borrow-asset usdt --initial-margin 0.5 --collateral-asset btc --collateral-price 100000 --precision 6
