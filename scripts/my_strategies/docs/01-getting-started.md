...
leverage: 20                      # 杠杆倍数（源码默认 50，建议新手下调）
...
> ℹ️ **提示**：默认情况下，`is_live_trading` 为 `false`，策略只会记录信号并发送通知，不会真正下单。如需实盘交易，请在确认风险后将其设置为 `true`。
...
> ✅ **细节提醒**：策略会自动忽略尚未闭合的最新 K 线（例如当前正在形成的 1 小时 K 线），确保所有判定基于完整数据。
...
4. **模拟模式**：
   - `is_live_trading` 默认为 `false`
   - 模拟模式下会在日志和通知中提示“当前为模拟模式，未执行真实下单”
   - 确认设置为 `true` 后重新加载配置
